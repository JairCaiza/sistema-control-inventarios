...
const obtenerMensajeError = (error: unknown): string => {
  if (axios.isAxiosError(error)) {
    ...
    if (error.response?.status === 403) {
      return "No tiene permisos para registrar salidas.";
    }

    if (error.response?.status === 404) {
      return "No existe una entrada previa para este empleado.";
    }

    if (error.response?.status === 409) {
      return "La salida no puede registrarse debido al estado actual de la asistencia.";
    }
  ...
  return "Ocurrió un error inesperado al registrar la salida.";
};
...
const RegistrarSalidaModal = ({
  ...
  const validarFormulario = (): boolean => {
    ...
    if (!form.fecha_hora) {
      nuevosErrores.fecha_hora = "Debe indicar la fecha y hora de salida.";
    } else {
      ...
      if (Number.isNaN(fecha.getTime())) {
        ...
        if (fecha.getTime() > ahora.getTime() + tolerancia) {
          nuevosErrores.fecha_hora =
            "No puede registrar una salida en una fecha futura.";
        }
      }
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const respuesta = await asistenciaService.registrarSalida(payload);

      const tiempo = respuesta.tiempo_trabajado?.texto;
          ...
          >
            ...
            <div>
              ...
              <p
                ...
                "
              >
                Registra la salida del empleado de la obra.
              </p>
            </div>
          </div>
            ...
            >
              ...
              <div>
                ...
                {errors.fecha_hora ? (
                  ...
                  >
                    <Clock3 size={13} />
                    Debe ser posterior a la última entrada registrada.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <textarea
                    ...
                    disabled={loadingSubmit}
                    rows={4}
                    placeholder="Ej.: Fin de jornada, salida por permiso, cambio de turno..."
                    className={`
                      w-full
                      resize-none
                      rounded-xl
...
export default RegistrarSalidaModal;
